...
print 'template <class classInst, class HistogramCreater> class Looper_bkg{'   
...
print 'template <class classInst,class HistogramCreater> void Looper_bkg<classInst,HistogramCreater>::Loop(TChain *tr,classInst& read,HistogramCreater hs,string ProcessName)\n'
...
print 'template <class classInst,class HistogramCreater> bool Looper_bkg<classInst,HistogramCreater>::Baseline(TChain *tr,classInst& read,string ProcessName,int iEvent){\n'
...
print 'template <class classInst,class HistogramCreater> bool Looper_bkg<classInst,HistogramCreater>::ProcessCuts(TChain *tr,classInst& read,string ProcessName,int iEvent){\n'
...
if len(data.list_Categories)>0:
    num=0
    for icate in data.list_Categories:
       print 'template <class classInst,class HistogramCreater> bool Looper_bkg<classInst,HistogramCreater>::%s(TChain *tr,classInst& read,string ProcessName,int iEvent){\n'%(icate)
...
for iname in data.list_VarsFill:
    if "UserDef_" in iname:
        print 'template <class classInst,class HistogramCreater> double Looper_bkg<classInst,HistogramCreater>::%s(TChain *tr,classInst& read,string ProcessName,int iEvent){\n'%(iname),
        print 'return 1.0;'
        print '}\n'
